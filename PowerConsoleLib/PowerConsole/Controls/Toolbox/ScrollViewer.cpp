...
ScrollViewer::ScrollViewer()
 :	// Dimensions
	...
	eventLayoutChanged().pop();

	eventContentAdded().add(bindEvent(ScrollViewer::onContentAdded));
	eventContentRemoved().add(bindEvent(ScrollViewer::onContentRemoved));
	eventLayoutChanged().add(bindEvent(ScrollViewer::onLayoutChanged));
	eventPaint().add(bindEvent(ScrollViewer::onPaint));
	eventMouseButtonGlobal().add(bindEvent(ScrollViewer::onMouseButtonGlobal));
	eventMouseMovedGlobal().add(bindEvent(ScrollViewer::onMouseMovedGlobal));
	eventMouseWheelGlobal().add(bindEvent(ScrollViewer::onMouseWheelGlobal));
	eventMouseWheel().add(bindEvent(ScrollViewer::onMouseWheel));
	eventTick().add(bindEvent(ScrollViewer::onTick));
}
...
// Settings
ScrollBarVisibilities ScrollViewer::getHorizontalScrollBarVisibility() const {
	return horizontalScrollBarVisibility;
}
void ScrollViewer::setHorizontalScrollBarVisibility(ScrollBarVisibilities horizontalScrollBarVisibility) {
	this->horizontalScrollBarVisibility = horizontalScrollBarVisibility;
	if (hasContent())
		updateScrollBars();
}
ScrollBarVisibilities ScrollViewer::getVerticalScrollBarVisibility() const {
	return verticalScrollBarVisibility;
}
void ScrollViewer::setVerticalScrollBarVisibility(ScrollBarVisibilities verticalScrollBarVisibility) {
	this->verticalScrollBarVisibility = verticalScrollBarVisibility;
	if (hasContent())
		updateScrollBars();
}
ScrollBarMouseWheelStates ScrollViewer::getMouseWheelState() const {
	return mouseWheelState;
}
void ScrollViewer::setMouseWheelState(ScrollBarMouseWheelStates mouseWheelState) {
	this->mouseWheelState = mouseWheelState;
}
bool ScrollViewer::isPositionKeptOnResize() const {
	return keepPositionOnResize;
}
void ScrollViewer::setPositionKeptOnResize(bool keepPosition) {
	this->keepPositionOnResize = keepPosition;
}
Point2I ScrollViewer::getMinThumbSize() const {
	return minThumbSize;
}
void ScrollViewer::setMinThumbSize(Point2I minThumbSize) {
	this->minThumbSize = GMath::max(Point2I::ONE, minThumbSize);
	if (hasContent())
		updateScrollBars();
}
Thickness ScrollViewer::getPadding() const {
	return padding;
}
void ScrollViewer::setPadding(Thickness padding) {
	this->padding = padding;
	updateScrollBars();
}
Point2I ScrollViewer::getIncrement() const {
	return increment;
}
void ScrollViewer::setIncrement(Point2I increment) {
	this->increment = increment;
}
Point2I ScrollViewer::getObjectOffset() const {
	return objectOffset;
}
void ScrollViewer::setObjectOffset(Point2I objectOffset) {
	this->objectOffset = objectOffset;
	updateScrollBars();
}

// State
bool ScrollViewer::isHorizontalActive() const {
	return horizontalActive;
}
bool ScrollViewer::isVerticalActive() const {
	return verticalActive;
}
bool ScrollViewer::canScrollLeft() const {
	return (viewPosition.x > 0);
}
bool ScrollViewer::canScrollRight() const {
	return (viewPosition.x < objectSize.x - viewportSize.x);
}
bool ScrollViewer::canScrollUp() const {
	return (viewPosition.y > 0);
}
bool ScrollViewer::canScrollDown() const {
...
Point2I ScrollViewer::getViewportSize() const {
	return viewportSize;
}
Point2I ScrollViewer::getObjectSize() const {
	return objectSize;
}
Point2I ScrollViewer::getViewPosition() const {
	return viewPosition;
}
void ScrollViewer::setViewPosition(Point2I position, bool relative) {
	if (!hasContent())
		return;
...
Point2I ScrollViewer::getScrollPosition() const {
	return scrollPosition;
}
void ScrollViewer::setScrollPosition(Point2I position, bool relative) {
	if (!hasContent())
		return;
...
void ScrollViewer::resizeScrollBars(Point2I newObjectSize) {
...
void ScrollViewer::updateScrollBars() {
	resizeScrollBars(hasContent() ? GMath::max(Point2I::ZERO, getContent()->getActualSize() + objectOffset) : Point2I::ZERO);
}
...
void ScrollViewer::updateContentLayout() {
	if (!hasContent())
		throw std::exception("'ScrollViewer::updateContentLayout()' should not be called when no content is set");
	//getContent()->setContainerLayout(Rectangle2I(-viewPosition, getActualSize()), Rectangle2I(viewportSize));
	Rectangle2I bounds = Rectangle2I(padding.topLeft() - viewPosition, getActualSize() - padding.horizontalVertical());
	...
	getContent()->setContainerLayout(
		bounds,
		(getLocalVisibleArea().intersect(Rectangle2I(padding.topLeft(), viewportSize)))
	);
}
...
Event<ScrollEventArgs>& ScrollViewer::eventScrolled() {
...
void ScrollViewer::onContentAdded(ControlSPtr sender, ContentModifiedEventArgs& e) {
	e.control->eventLayoutChanged().add(bindEvent(ScrollViewer::onContentLayoutChanged));
	updateScrollBars();
}
void ScrollViewer::onContentRemoved(ControlSPtr sender, ContentModifiedEventArgs& e) {
	e.control->eventLayoutChanged().remove(unbindEvent(ScrollViewer::onChildLayoutChanged));
	updateScrollBars();
}
void ScrollViewer::onLayoutChanged(ControlSPtr sender, LayoutChangedEventArgs& e) {
...
void ScrollViewer::onContentLayoutChanged(ControlSPtr sender, LayoutChangedEventArgs& e) {
	if (e.sizeChanged) {
		updateScrollBars();
	}
}
void ScrollViewer::onPaint(ControlSPtr sender, PaintEventArgs& e) {
	auto g = e.graphics;
...
void ScrollViewer::onMouseButtonGlobal(ControlSPtr sender, MouseButtonEventArgs& e) {
...
void ScrollViewer::onMouseMovedGlobal(ControlSPtr sender, MouseEventArgs& e) {
	hovering = ScrollBarHovering::None;
	if (horizontalActive) {
		if (dragging == ScrollBarDragging::None) {
...
void ScrollViewer::onMouseWheel(ControlSPtr sender, MouseWheelEventArgs& e) {
...
void ScrollViewer::onMouseWheelGlobal(ControlSPtr sender, MouseWheelEventArgs& e) {
	if (mouseWheelState == ScrollBarMouseWheelStates::Global) {
		onMouseWheel(sender, e);
	}
}
void ScrollViewer::onTick(ControlSPtr sender, TickEventArgs& e) {
