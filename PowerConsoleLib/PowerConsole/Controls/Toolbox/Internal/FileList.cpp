...
#define directory loader.getDirectory()
...
FileList::FileList() 
 :	// Items
	...
	eventTick().add(bindEvent(FileList::onTick));
	...
	loader.setFirstTimeoutPeriod(200);
	loader.setTimeoutPeriod(6);
	loader.setKeepFileFunction([this](const FileInfo& file) {
		if (!allowReadOnly && file.isReadOnly())
			return false;
		if (!allowSystem && file.isSystem())
			return false;
		bool hasExtension = filters.empty() || (file.fileType != FileTypes::File && file.shortcutType != FileTypes::File);
		for (const std::string& filter : filters) {
			if (file.fileType == FileTypes::File) {
				if (compareString(file.extension, filter, false)) {
					hasExtension = true;
					break;
				}
			}
			else if (file.shortcutType == FileTypes::File) {
				if (compareString(Path::getExtension(file.shortcutPath), filter, false)) {
					hasExtension = true;
					break;
				}
			}
	...
	loader.setFileAddedFunction([this](int index, FileInfo& file) {
		updateScrollBars();
		if (file.fileType == FileTypes::File && (bool)functionLoadFileDetails) {
...
std::string FileList::getResolvedCurrentDirectory() const {
	if (directory.directoryFile.fileType == FileTypes::Shortcut && directory.directoryFile.shortcutType == FileTypes::File) {
		return Path::getDirectory(directory.directoryFile.shortcutPath);
	}
	else {
		return directory.directoryFile.path;
	}
...
void FileList::deleteFile() {
	loader.remove(selection);
	updateScrollBars();
	selection--;
}
void FileList::renameFile(const std::string& newName) {
	std::string newPath = Path::combine(Path::getDirectory(directory.fileList[selection].path), newName);
	loader.remove(selection);
	addFile(newPath);
	updateScrollBars();
}
void FileList::addFile(PcIO::FileInfo&& file) {
	int newSelection = loader.insert(std::move(file));
...
void FileList::addFile(const std::string& path) {
	FileInfo file;
	if (file.load(path)) {
...
void FileList::onPaint(ControlSPtr sender, PaintEventArgs& e) {
	...
	const unsigned char borderHoverColor = toColor(BLACK, GRAY);

	const unsigned char computerColor = toColor(BLACK, DARK_CYAN);
	const unsigned char parentColor = toColor(BLACK, DARK_GREEN);
	const unsigned char folderColor = toColor(BLACK, DARK_YELLOW);
	const unsigned char driveColor = toColor(BLACK, DARK_MAGENTA);
	const unsigned char shortcutColor = toColor(BLACK, CYAN);
	...
	const unsigned char errorHoverColor = toColor(DARK_RED, WHITE);
	const unsigned char errorPressColor = toColor(DARK_RED, DARK_GRAY);
		...
		if (file.fileType == FileTypes::File) {
			for (int i = 1; i < (int)columns.size(); i++) {
				...
				case DETAIL_FILE_MODIFIED:
					drawString = Path::getFileTimeString(file.lastModified, true, false, true, false);
					if (maxSize > 0) {
				...
				case DETAIL_FILE_CREATED:
					drawString = Path::getFileTimeString(file.creationDate, true, false, true, false);
					if (maxSize > 0) {
						if ((int)drawString.length() > detail.width - 2)
							g->drawString(Point2I(offset, y), drawString.substr(0, detail.width - 3) + "..", Pixel(' ', itemColor));
						else
				...
				default:
					if ((bool)functionDrawFileDetail && !file.isInvalid()) {
						DrawFileDetailArgs args = DrawFileDetailArgs(detail.id, &file);
			...
			if (file.fileSize != 0UL) {
				const unsigned char driveBarColor = toColor(BLACK, DARK_CYAN);
				const unsigned char driveBackColor = toColor(BLACK, DARK_CYAN);
...
void FileList::onTick(ControlSPtr sender, TickEventArgs& e) {
	if (loader.isLoading()) {
