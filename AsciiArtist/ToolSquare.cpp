...
//=================================================================|
#pragma region ToolSquare
//========= CONSTRUCTORS =========
#pragma region Constructors

PowerConsole::AsciiArtistApp::ToolSquare::ToolSquare() : Tool(ToolTypes::Square, "Square") {
	this->_secondary		= false;
	this->_startPoint		= Coord::Zero;
}
PowerConsole::AsciiArtistApp::ToolSquare::~ToolSquare() {
	
}
void PowerConsole::AsciiArtistApp::ToolSquare::Initialize(AsciiArtistManager* aa, ConsoleApp* app, AsciiAnimation* drawingBuffer) {
	Tool::Initialize(aa, app, drawingBuffer);
}
void PowerConsole::AsciiArtistApp::ToolSquare::Uninitialize() {
	Tool::Uninitialize();
}
...
void PowerConsole::AsciiArtistApp::ToolSquare::Update() {
	...
	if (!_drawing) {
		...
		if ((_app->GetMouse()->IsButtonPressed(MouseButtons::Left) && _secondary) ||
			...
			_drawingBuffer->GetFrame(0).DrawRect(_startPoint, endPoint - _startPoint, drawingPixel, !_aa->IsShapeOutline(), false, attributes);
			...
			_drawingBuffer->GetFrame(1).DrawRect(_startPoint, endPoint - _startPoint, drawingPixel, !_aa->IsShapeOutline(), false, attributes);
			...
			drawingPixel.Color = _aa->GetImage()->GetBackground().Color;
			drawingPixel.Attributes = attributes;
			_drawingBuffer->GetFrame(2).DrawRect(_startPoint, endPoint - _startPoint, drawingPixel, !_aa->IsShapeOutline(), false, attributes, PixelOptions::OptionCopySpecial);
			...
			drawingPixel.Color = _aa->GetImage()->GetBackground().Color;
			drawingPixel.Attributes = attributes;
			_drawingBuffer->GetFrame(3).DrawRect(_startPoint, endPoint - _startPoint, drawingPixel, !_aa->IsShapeOutline(), false, attributes, PixelOptions::OptionCopySpecial);
		}
	}
...
void PowerConsole::AsciiArtistApp::ToolSquare::Activate() {
	
}
void PowerConsole::AsciiArtistApp::ToolSquare::Finalize() {
	if (_drawing) {
		UndoAction* undo = new UndoAction("Square", Actions::Draw, _aa->GetCurrentFrame());
...
void PowerConsole::AsciiArtistApp::ToolSquare::Cancel() {
	if (_drawing) {
		_drawing = false;
	}
}
...
void PowerConsole::AsciiArtistApp::ToolSquare::Draw(AsciiImageBase& g) {
