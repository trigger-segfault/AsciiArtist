...
//=================================================================|
#pragma region ToolCircle
//========= CONSTRUCTORS =========
#pragma region Constructors

PowerConsole::AsciiArtistApp::ToolCircle::ToolCircle() : Tool(ToolTypes::Circle, "Circle") {
	this->_secondary		= false;
	this->_startPoint		= Coord::Zero;
}
PowerConsole::AsciiArtistApp::ToolCircle::~ToolCircle() {
	
}
void PowerConsole::AsciiArtistApp::ToolCircle::Initialize(AsciiArtistManager* aa, ConsoleApp* app, AsciiAnimation* drawingBuffer) {
	Tool::Initialize(aa, app, drawingBuffer);
}
void PowerConsole::AsciiArtistApp::ToolCircle::Uninitialize() {
	Tool::Uninitialize();
}
...
void PowerConsole::AsciiArtistApp::ToolCircle::Update() {
	...
	if (!_drawing) {
		...
		if ((_app->GetMouse()->IsButtonPressed(MouseButtons::Left) && _secondary) ||
			...
			_drawingBuffer->GetFrame(0).DrawEllipse(_startPoint, endPoint - _startPoint, drawingPixel, !_aa->IsShapeOutline(), false, attributes);
			...
			_drawingBuffer->GetFrame(1).DrawEllipse(_startPoint, endPoint - _startPoint, drawingPixel, !_aa->IsShapeOutline(), false, attributes);
			...
			drawingPixel.Color = _aa->GetImage()->GetBackground().Color;
			drawingPixel.Attributes = attributes;
			_drawingBuffer->GetFrame(2).DrawEllipse(_startPoint, endPoint - _startPoint, drawingPixel, !_aa->IsShapeOutline(), false, attributes, PixelOptions::OptionCopySpecial);
			...
			drawingPixel.Color = _aa->GetImage()->GetBackground().Color;
			drawingPixel.Attributes = attributes;
			_drawingBuffer->GetFrame(3).DrawEllipse(_startPoint, endPoint - _startPoint, drawingPixel, !_aa->IsShapeOutline(), false, attributes, PixelOptions::OptionCopySpecial);
		}
	}
...
void PowerConsole::AsciiArtistApp::ToolCircle::Activate() {
	
}
void PowerConsole::AsciiArtistApp::ToolCircle::Finalize() {
	if (_drawing) {
		UndoAction* undo = new UndoAction("Circle", Actions::Draw, _aa->GetCurrentFrame());
...
void PowerConsole::AsciiArtistApp::ToolCircle::Cancel() {
	if (_drawing) {
		_drawing = false;
	}
}
...
void PowerConsole::AsciiArtistApp::ToolCircle::Draw(AsciiImageBase& g) {
