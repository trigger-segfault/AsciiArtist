...
PowerConsole::AsciiArtistApp::ToolPen::ToolPen() : Tool(ToolTypes::Pen, "Pen") {
	this->_secondary		= false;
	this->_lastPoint		= Coord::Zero;
}
PowerConsole::AsciiArtistApp::ToolPen::~ToolPen() {
	
}
void PowerConsole::AsciiArtistApp::ToolPen::Initialize(AsciiArtistManager* aa, ConsoleApp* app, AsciiAnimation* drawingBuffer) {
	Tool::Initialize(aa, app, drawingBuffer);
}
void PowerConsole::AsciiArtistApp::ToolPen::Uninitialize() {
	Tool::Uninitialize();
}
...
void PowerConsole::AsciiArtistApp::ToolPen::Update() {
	...
	if (!_drawing) {
		...
		if (pressed) {
			_drawing = true;
			_lastPoint = _aa->GetCanvas()->GetImageMousePosition();
		...
		if ((_app->GetMouse()->IsButtonPressed(MouseButtons::Left) && _secondary) ||
			...
			Coord newPoint = _aa->GetCanvas()->GetImageMousePosition();
			...
			_drawingBuffer->GetFrame(0).DrawLine(_lastPoint, newPoint - _lastPoint, drawingPixel, false, attributes);
			...
			_drawingBuffer->GetFrame(1).DrawLine(_lastPoint, newPoint - _lastPoint, drawingPixel, false, attributes);
			...
			drawingPixel.Color = _aa->GetImage()->GetBackground().Color;
			drawingPixel.Attributes = attributes;
			_drawingBuffer->GetFrame(2).DrawLine(_lastPoint, newPoint - _lastPoint, drawingPixel, false, attributes, PixelOptions::OptionCopySpecial);
			...
			drawingPixel.Color = _aa->GetImage()->GetBackground().Color;
			drawingPixel.Attributes = attributes;
			_drawingBuffer->GetFrame(3).DrawLine(_lastPoint, newPoint - _lastPoint, drawingPixel, false, attributes, PixelOptions::OptionCopySpecial);
...
void PowerConsole::AsciiArtistApp::ToolPen::Activate() {
	
}
void PowerConsole::AsciiArtistApp::ToolPen::Finalize() {
	if (_drawing) {
		UndoAction* undo = new UndoAction("Pen", Actions::Draw, _aa->GetCurrentFrame());
...
void PowerConsole::AsciiArtistApp::ToolPen::Cancel() {
	if (_drawing) {
		_drawing = false;
	}
}
...
void PowerConsole::AsciiArtistApp::ToolPen::Draw(AsciiImageBase& g) {
