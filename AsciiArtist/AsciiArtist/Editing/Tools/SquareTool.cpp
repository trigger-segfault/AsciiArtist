...
SquareTool::SquareTool(CanvasSPtr canvas)
	: Tool(canvas, "Square"),
	secondary(false),
...
fixed_array<std::string> SquareTool::getUsedOptions() const {
	return fixed_array<std::string>{"Overwrte", "Outline"};
}
Point2I SquareTool::getSelectionSize() const {
...
void SquareTool::drawChanges() {
	...
	g->drawRectangle(rectangle, canvas->getDrawingPixel(secondary), !canvas->getOption("Outline"));
}
...
void SquareTool::onMouseButton(MouseButtonEventArgs e) {
	auto canvas = this->canvas.lock();
...
void SquareTool::onMouseMoved(MouseEventArgs e) {
	drawChanges();
}
void SquareTool::onKey(KeyboardEventArgs e) {
	shift = getFlag(e.modifiers, Modifiers::Shift);
	drawChanges();
}
void SquareTool::onPaint(PaintEventArgs e) {
	auto canvas = this->canvas.lock();
	auto g = e.graphics;
