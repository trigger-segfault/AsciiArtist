...
////////////////////////////////////////////////////////////
void SoundStream::initialize(unsigned int channelCount, unsigned int sampleRate, Uint64 sampleCount)
{
...
void SoundStream::stop()
{
    ...
    onSeek(Time::Zero);
	if (!m_samples.empty())
		memset(&m_samples[0], 0, m_samples.size() * sizeof(Int16));
...
Uint64 SoundStream::getSampleCount() const {
...
const Int16* SoundStream::getSamples() const {
	return &m_samples[0];
}
int SoundStream::getAvailableSamples() const {
	Uint64 samplePos = (int)(getPlayingOffset().asSeconds() * m_sampleRate * m_channelCount);
	return m_samplePosition - samplePos;
}
...
bool SoundStream::fillAndPushBuffer(unsigned int bufferNum, bool immediateLoop)
{
    ...
    Chunk data = {NULL, 0};
	Uint64 samplePosition = 0;
    for (Uint32 retryCount = 0; !onGetData(data, samplePosition) && (retryCount < BufferRetries); ++retryCount)
    {
    ...
    if (data.samples && data.sampleCount)
    {
		Uint64 sampleStart = samplePosition - data.sampleCount;
		size_t tempSampleCount = data.sampleCount;
		if (samplePosition > m_sampleCount) {
			tempSampleCount -= (samplePosition - m_sampleCount);
			samplePosition = m_sampleCount;
		}
		if (sampleStart < m_sampleCount) {
			memcpy_s(&m_samples[sampleStart], (rsize_t)(tempSampleCount * sizeof(Int16)), data.samples, (rsize_t)(tempSampleCount * sizeof(Int16)));
		}
