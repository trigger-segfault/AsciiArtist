...
namespace
{
    ...
    void ensureDefaultReadersWritersRegistered()
    {
        ...
        if (!registered)
        {
            ...
            sf::SoundFileFactory::registerReader<sf::priv::SoundFileReaderFFmpeg>();
            registered = true;
        }
    }
}
...
SoundFileReader* SoundFileFactory::createReaderFromFilename(const std::wstring& filename) {
	// Register the built-in readers/writers on first call
	ensureDefaultReadersWritersRegistered();
	...
	FileInputStream stream;
	if (!stream.open(filename)) {
		err() << "Failed to open sound file \"" << "unicode" << "\" (couldn't open stream)" << std::endl;
		return NULL;
	}
	...
	// No suitable reader found
	err() << "Failed to open sound file \"" << "unicode" << "\" (format not supported)" << std::endl;
	return NULL;
}
