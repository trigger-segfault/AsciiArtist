...
bool InputSoundFile::openFromFile(const std::string& filename)
{
    ...
    m_reader = (sf::priv::SoundFileReaderFFmpeg*)SoundFileFactory::createReaderFromFilename(filename);
    if (!m_reader)
        return false;
	...
	//if (!m_reader->open(filename, info))
...
bool InputSoundFile::openFromFile(const std::wstring& filename) {
	// If the file is already open, first close it
	close();
...
bool InputSoundFile::openFromMemory(const void* data, std::size_t sizeInBytes)
{
    ...
    // Find a suitable reader for the file type
    m_reader = (sf::priv::SoundFileReaderFFmpeg*)SoundFileFactory::createReaderFromMemory(data, sizeInBytes);
    if (!m_reader)
        return false;
...
bool InputSoundFile::openFromStream(InputStream& stream)
{
    ...
    // Find a suitable reader for the file type
    m_reader = (sf::priv::SoundFileReaderFFmpeg*)SoundFileFactory::createReaderFromStream(stream);
    if (!m_reader)
        return false;
